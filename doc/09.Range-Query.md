...
第一步是在B-树中添加范围查询功能。BIter类型使我们能够以迭代的方式遍历B-树。 
...
nodeLookupLE函数仅适用于范围查询中的“小于或等于”操作符，对于其他三个操作符（小于；大于；大于或等于），结果可能会偏差一个位置。我们将在BTree.Seek函数中修复这个问题。
...
为了支持范围查询，键值存储中的序列化主键必须能被正确比较。实现这一点的一种方法是反序列化主键，然后按列进行比较。我们将采用另一种方法，即让序列化的键字节反映出它们的字典序，也就是说，键可以通过`bytes.Compare`或`memcmp`直接比较而无需先反序列化。我们称这种技术为“保持顺序的编码”，它可以在不控制底层键值存储的键比较函数的情况下使用。
